...
interface GridControlsState {
  // Define the state properties of GridControls here
...
export default class MobileGridControls extends GridControls {
  ...
  constructor(props: MobileGridControlsProps) {
    super(props);
    this.state = {
  ...
  componentDidUpdate(prevProps: MobileGridControlsProps, prevState: MobileGridControlsState) {
  ...
  fitOnScreen(fitCurrentClue?: boolean): void {
    if (!fitCurrentClue && this.state.lastFitOnScreen && this.state.lastFitOnScreen > Date.now() - 100)
      return;
  ...
  handleClueBarTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
  ...
  handleClueBarTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    let stateUpdates: Partial<MobileGridControlsState> = {
      touchingClueBar: false,
    };

    if (!this.previewClue) {
      // nothing to do if cannot perform action :|
    } else if (clueBarGesture.x !== undefined && Math.abs(clueBarGesture.x) > 50) {
      ...
      clueBarGesture.y !== undefined &&
      !clueBarGesture.x &&
      (Math.abs(clueBarGesture.y) > 20 || Math.abs(clueBarGesture.y) <= 3)
    ) {
      this.flipDirection();
      stateUpdates = {
        ...stateUpdates,
  ...
  handleClueBarTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    if (!this.props.enableClueBarGestures) return;
    e.preventDefault();
    const touch = e.touches[0];
    this.setState({
  ...
  handleLeftArrowTouchEnd = (e: React.TouchEvent<HTMLDivElement>) => {
    e.preventDefault();
